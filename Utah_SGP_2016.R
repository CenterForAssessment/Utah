...
# Utah_Data_LONG_2016.Rdata
# Utah_SGP.Rdata
##### Data/
# Utah_SGP_LONG_Data.txt
# Utah_SGP_LONG_Data_2016.txt
...
##    	   In 2016 this also includes non_FAY student data from 2008-2014.

# require(RODBC)
# dbhandle <- odbcDriverConnect('driver={SQL Server}; server=acsdbstage; database=be_upass;')

# Utah_Data_LONG_2016 <- sqlQuery(dbhandle, 'SELECT * FROM v_sgp_longfile_20160811 - SY2016 - 2016 Accountability Only')

# write.csv(Utah_Data_LONG_2016, file='SGP_Longfile_2016.csv', row.names=FALSE)


# 6. DATA CLEANING AND PREP
#    SEE THE R SCRIPT Utah_Data_LONG_2016.R FOR DETAILS.
...
source("SGP_CONFIG/EOGT/UT_EOGT_2016.R")
...
Utah_SGP <- updateSGP(
	...
	with_sgp_data_LONG = Utah_Data_LONG_2016,
...
###  Load 2016 data produced in outputSGP step:
load("Data/Utah_SGP_LONG_Data_2016.Rdata")
...
setkeyv(Utah_SGP_LONG_Data_2016, c("VALID_CASE", "CONTENT_AREA", "ID"))
...
Target_Table <- unique(Utah_SGP_LONG_Data_2016_FORMATTED)[,list(GRADE, CONTENT_AREA, TARGET_CONTENT_AREA, SCALE_SCORE, TARGET_SCALE_SCORE)]
...
write.table(Target_Table, file="/media/Data/Dropbox (SGP)/Github_Repos/Projects/Utah/Misc/Utah_SGP_Scale_Score_Target_Table_2016.csv", sep=",", row.names=FALSE, na="", quote=FALSE)
